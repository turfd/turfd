...
import {
  DUCK_HEIGHT_PX,
  DUCK_KNOCKBACK_DECAY_PER_SEC,
  DUCK_KNOCKBACK_GROUND_VY_PX,
  DUCK_KNOCKBACK_HORIZONTAL_CAP_PX,
  DUCK_KNOCKBACK_RESISTANCE_PERCENT,
  DUCK_KNOCKBACK_SPRINT_MULT,
  DUCK_PANIC_DURATION_SEC,
  DUCK_PANIC_FLIP_INTERVAL_SEC,
  DUCK_PANIC_SPEED_PX,
  DUCK_WALK_SPEED_PX,
  DUCK_WANDER_INTERVAL_SEC_MAX,
  DUCK_WANDER_INTERVAL_SEC_MIN,
  DUCK_WATER_BUOYANCY_ACCEL_PX,
  DUCK_WATER_GRAVITY_MULT,
  DUCK_WATER_MAX_SINK_SPEED_PX,
  DUCK_WATER_MAX_UPWARD_SPEED_PX,
  DUCK_WATER_SPEED_MULT,
  DUCK_WIDTH_PX,
  MOB_GRAVITY_PX,
  MOB_TERMINAL_VY_PX,
  mobSwimBobVyDelta,
...
function feetToScreenAABB(x: number, y: number, w: number, h: number): AABB {
...
function duckAabbOverlapsWater(world: World, pos: { x: number; y: number }): boolean {
  if (!world.getRegistry().isRegistered("stratum:water")) {
    return false;
  }
  const waterId = world.getWaterBlockId();
  const region = feetToScreenAABB(pos.x, pos.y, DUCK_WIDTH_PX, DUCK_HEIGHT_PX);
...
export function tickDuckPhysics(
  world: World,
  m: MobDuckState,
  dt: number,
  ...
  if (m.panicRemainSec > 0) {
    ...
    if (m.panicFlipTimerSec <= 0) {
      m.panicFlipTimerSec = DUCK_PANIC_FLIP_INTERVAL_SEC;
      m.facingRight = rng.nextFloat() < 0.5;
    }
    m.targetVx = (m.facingRight ? 1 : -1) * DUCK_PANIC_SPEED_PX;
  } else if (m.wanderTimerSec <= 0) {
    m.wanderTimerSec =
      DUCK_WANDER_INTERVAL_SEC_MIN +
      rng.nextFloat() * (DUCK_WANDER_INTERVAL_SEC_MAX - DUCK_WANDER_INTERVAL_SEC_MIN);
    const roll = rng.nextFloat();
    if (roll < 0.35) {
      m.targetVx = 0;
    } else {
      m.targetVx = (rng.nextFloat() < 0.5 ? -1 : 1) * DUCK_WALK_SPEED_PX;
      m.facingRight = m.targetVx > 0;
    }
  }

  const inWater = duckAabbOverlapsWater(world, m);
  m.hitKnockVx *= Math.exp(-DUCK_KNOCKBACK_DECAY_PER_SEC * dt);
  if (Math.abs(m.hitKnockVx) < 3) {
    m.hitKnockVx = 0;
  }
  const baseWalkVx = m.targetVx * (inWater ? DUCK_WATER_SPEED_MULT : 1);
  ...
  let mover = feetToScreenAABB(m.x, m.y, DUCK_WIDTH_PX, DUCK_HEIGHT_PX);
  const dx = m.vx * dt;
  const dy = m.vy * dt;
  ...
  const feet = screenAABBTofeet(mover, DUCK_WIDTH_PX, DUCK_HEIGHT_PX);
  m.x = feet.x;
  m.y = feet.y;
  if (hitX) {
    m.vx = 0;
  ...
  m.inWater = duckAabbOverlapsWater(world, m);
  if (Math.abs(m.vx) > 0.5) {
    m.facingRight = m.vx > 0;
  }
}

export function applyDuckPanic(m: MobDuckState, fromX: number): void {
  m.panicRemainSec = DUCK_PANIC_DURATION_SEC;
  m.panicFlipTimerSec = 0;
  const away = m.x >= fromX ? 1 : -1;
  m.facingRight = away > 0;
  m.targetVx = away * DUCK_PANIC_SPEED_PX;
}

export function applyDuckKnockback(
  m: MobDuckState,
  fromX: number,
  horizontalBasePx: number,
  opts?: { sprintKnockback?: boolean },
): void {
  const r = DUCK_KNOCKBACK_RESISTANCE_PERCENT / 100;
  let h = horizontalBasePx * (1 - r);
  if (opts?.sprintKnockback === true) {
    h *= DUCK_KNOCKBACK_SPRINT_MULT;
  }
  const away = m.x >= fromX ? 1 : -1;
  m.hitKnockVx += away * h;
  const cap = DUCK_KNOCKBACK_HORIZONTAL_CAP_PX;
  m.hitKnockVx = Math.max(-cap, Math.min(cap, m.hitKnockVx));
  m.facingRight = away > 0;
  if (m.onGround && !m.inWater) {
    m.vy -= DUCK_KNOCKBACK_GROUND_VY_PX;
  }
}
