/** Furnace tile serialization (IndexedDB + network chunk tail). Uses numeric item ids. */
...
/** Slot shape in JSON / binary (optional damage for tools). */
export type FurnacePersistedSlot = {
  id: number;
  count: number;
  damage?: number;
} | null;
...
export function furnaceStackFromPersisted(s: FurnacePersistedSlot): FurnaceStack {
  if (s === null || s.count <= 0) {
    return null;
  }
  const d = s.damage;
  if (typeof d === "number" && d > 0 && Number.isFinite(d)) {
    return { itemId: s.id as ItemId, count: s.count, damage: Math.floor(d) };
  }
  return { itemId: s.id as ItemId, count: s.count };
}

export function furnaceStackToPersisted(s: FurnaceStack): FurnacePersistedSlot {
  if (s === null || s.count <= 0) {
    return null;
  }
  const d = s.damage ?? 0;
  if (d > 0) {
    return { id: s.itemId as number, count: s.count, damage: d };
  }
...
function coerceSlot(v: unknown): FurnacePersistedSlot {
  ...
  const damage = (v as { damage?: number }).damage;
...
export function normalizeFurnacePersistedChunk(raw: unknown): FurnacePersistedChunk | undefined {
  ...
  if ("input" in o && "output" in o) {
    return migrateLegacyFurnacePersisted({
      lx,
      ly,
      input: coerceSlot(o.input),
      fuel: coerceSlot(o.fuel),
      output: coerceSlot(o.output),
      cookProgressSec: num(o.cookProgressSec),
      fuelRemainingSec: num(o.fuelRemainingSec),
      lastProcessedWorldTimeMs: num(o.lastProcessedWorldTimeMs),
    });
...
export function furnaceTileToPersisted(
  ...
  const outputSlots = state.outputSlots.map((s) => furnaceStackToPersisted(s));
  ...
  return {
    ...
    fuel: furnaceStackToPersisted(state.fuel),
...
export function persistedToFurnaceTile(p: FurnacePersistedChunk): FurnaceTileState {
  const outputSlots: FurnaceStack[] = [];
  for (let i = 0; i < FURNACE_OUTPUT_SLOT_COUNT; i++) {
    const s = p.outputSlots[i];
    outputSlots.push(furnaceStackFromPersisted(s ?? null));
  }
  ...
  return {
    outputSlots,
    fuel: furnaceStackFromPersisted(p.fuel),
...
function writeSlotV2(
  ...
  if (slot === null || slot.count <= 0 || slot.id <= 0) {
    view.setUint16(o, 0, true);
    view.setUint16(o + 2, 0, true);
  } else {
    view.setUint16(o, slot.id & 0xffff, true);
    view.setUint16(o + 2, slot.count & 0xffff, true);
  }
  return o + 4;
}
...
function writeSlotV3(
  ...
  if (slot === null || slot.count <= 0 || slot.id <= 0) {
    ...
    view.setUint16(o + 2, slot.count & 0xffff, true);
    const dmg =
      typeof slot.damage === "number" && slot.damage > 0
        ? Math.min(0xffff, Math.floor(slot.damage))
        : 0;
