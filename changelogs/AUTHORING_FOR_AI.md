...
- **Dev preview**: with `npm run dev`, **`http://localhost:5173/stratum/update`** shows an **in-game GFM** column and a **Discord-style** column; both **refresh as you type** (debounced). The Discord column uses the same **embed builder** as the webhook but **always includes live markdown** descriptions so edits are visible. **Banner / footer** images still come from **`scripts/discordChangelogImageUrls.ts`**. The **`mainEmbedImageUrl`** (bottom embed: patch notes as a PNG) is **not** shown in the tool; CI sends **two image embeds** (banner + that URL) unless **`includeMarkdownWhenMainImageSet: true`**. Optional `.env.local`: `DISCORD_CHANGELOG_EMBED_COLOR` (decimal RGB) for the preview stripe.
- **CI / Supabase naming**: store the webhook as **`DISCORD_WEBHOOK_URL_CHANGELOG`** (GitHub Actions repository secret and/or Supabase secret). Enable posting with GitHub repository **variable or secret** **`DISCORD_CHANGELOG_POST`** = `true` (prefer Variables). Optional **`DISCORD_CHANGELOG_DEDUPE_BUSTER`** as variable or secret bumps the dedupe key to force a repost for the same notes.
