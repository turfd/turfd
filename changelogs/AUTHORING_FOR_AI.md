...
- **Dev preview**: with `npm run dev`, **`http://localhost:5173/stratum/update`** shows an **in-game GFM** column and a **Discord-style** column; both **refresh as you type** (debounced). The Discord column uses the same **embed builder** as the webhook but passes **`mainEmbedImageUrl: undefined`** so descriptions stay live from the form. **CI** uses committed URLs: banner embed, then bottom embed = **image + `description`** from `[Summary]` / `[Changes]` (unless **`omitTextOnMainImageEmbed`**). Optional `.env.local`: `DISCORD_CHANGELOG_EMBED_COLOR` (decimal RGB) for the preview stripe.
