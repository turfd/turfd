...
- **Dev preview**: with `npm run dev`, **`http://localhost:5173/stratum/update`** shows an **in-game GFM** column and a **Discord-style** column built from the same JSON the webhook uses. Optional `.env.local` (never `VITE_*`): `DISCORD_CHANGELOG_HEADER_IMAGE_URL`, `DISCORD_CHANGELOG_MAIN_EMBED_IMAGE_URL`, `DISCORD_CHANGELOG_FOOTER_IMAGE_URL`, `DISCORD_CHANGELOG_EMBED_COLOR` (decimal RGB integer) so the right column matches your banner layout.
